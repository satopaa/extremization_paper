...
\begin{enumerate}
\item[] \textit{Generated Estimate:} Based on the first game, the $i$th expert believes that RED has an independent chance of $q_i$ winning any of the two remaining games. This probability is assumed to arise from a probability distribution defined on the unit interval. Therefore any individual differences in the way the experts process the first game and turn the acquired information into a probability $q_i$ are assumed to arise from the probability distribution. The $i$th expert reports
\begin{align}
p_i &= \begin{cases}
...
\item[] \textit{Interpreted Estimate:} Interpretations are different ways of seeing the first game. Assume that RED's performance in the first game can be attributed entirely to its \textit{defense} $D$ and \textit{offense} $O$ that are equally likely to be either good 1 or bad -1. Expert 1 follows only the defense and Expert 2 looks only at the offensive play. Based on these attributes the experts construct their final predictive models. If RED wins a game when $D + 2O \geq 2$, the experts report (\ref{basisP}) but with
\begin{align*}
q_1 &= \begin{cases}
1/2 & \text{ if } D = 1\\
0 & \text{ otherwise }
...
%
%
%It is not necessary for the experts to have the same knowledge. This example illustrates that even under the same information, they can choose to give different probability forecasts. The differences then arise from their personal interpretations of the importance of the first game.

\item[] \textit{Information Theoretic Estimate:} The experts know that RED has a 1/2 chance of winning any given game. If expert 1 knows 2/3 of the full information, i.e. the value of $X_1 = G_j + G_k$ for some $j,k \in \{1, 2, 3\}$ with $j \neq k$, then it is optimal to report 
\begin{align*}
p_1 &= \begin{cases}
0 & \text{ if } X_1 = -2\\
1/2 & \text{ if } X_1 = 0\\
...
\end{cases}
\end{align*}
If expert 2 only knows 1/3 of the full information, i.e. the value of $X_2 = P_j$ for some $j \in \{1, 2, 3\}$, then 
\begin{align*}
p_2 &= \begin{cases}
1/4 & \text{ if } X_2 = -1\\
3/4 & \text{ if } X_2 = 1\\
\end{cases}
\end{align*}
If all the information known by expert 2 is also known by expert 1, their forecasts are positively correlated. In this example the correlation would be $\sqrt{2}/2 \approx 0.71$.
...
Based on Observation \ref{infoObservation} there is a monotonic and positive relationship between $\alpha$ and $\delta_0$. This means that the more the sample average is extremized the more information its corresponding $\boldsymbol{X}$ contains, and \textit{vice versa}. Observation \ref{infoObservation}  is interesting for two reasons: (a) it allows the researcher to use black-box models from existing literature to determine the extremizing constant and then use it to analyze the amount of information in $\boldsymbol{X}$, and (b) it allows us to easily show that $\alpha \geq 1$ under any information structure.
