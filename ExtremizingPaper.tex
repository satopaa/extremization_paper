...
%More specifically, a probability aggregation problem consist of a target event and a set of probability forecasts.  The forecasts are assumed to be informative and hence correlated with the event. However, to synthesize the forecasts into a single accurate prediction, it is necessary to assume a structure for the intercorrelation among the forecasts and the target event.. 
...
%When $\lambda = 1.0$, the location $x_0 = 1.0$ and scale $\gamma = 0.0$, which represents a point mass at $\alpha = 1.0$.  In this case, all information is public, and the group of experts is as good as a single expert. Therefore, given that the forecast of a group with only one expert coincides with the crowd belief, $\lambda = 1.0$ always implies $\alpha = 1.0$ regardless of the value of $\delta$.
...
%In Figure \ref{LevelplotsOracle} the boarder line is defined by $\delta' = \delta(2- \lambda) = 1$. 
...
%Based on Figure \ref{probOracle}, the probit opinion pool is more likely to agree with the crowd belief at points closer to the end points of the boarder line, namely, when $(\delta, \lambda) = (0.5, 0.0)$ or  $(\delta, \lambda) = (1.0, 1.0)$. In between these two points,  there is a chance that the probit opinion pool disagrees with the crowd belief on the more likely outcome of the target event. 
%The correction requires a large but negative value of $\alpha$. Consequently, the scale $\gamma$ increases and $\P(\alpha > 1 | \Sigma_{22}, \delta')$ decreases slightly.  
...
%In Figure \ref{LevelplotsOracle} the location parameter $x_0 \geq 1$ and, consequently, $\P(\alpha > 1 | \Sigma_{22}, \delta') \in (0.5, 1.0]$. This follows from Proposition \ref{positiveProbThm}. 
