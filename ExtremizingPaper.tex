...
theoretical framework for forecast aggregation called the \textit{partial information framework}. Under this framework, forecast heterogeneity stems from information available to the forecasters and how they decide to use it. For instance, two forecasters may study the same books regarding modern Egypt, but end up utilizing different  portions of the information in their predictions. 
...
estimator of an indicator function $\one_A$ that equals one or zero depending whether the target event
...
indicators.  Furthermore, if two forecasters have access to the same $\sigma$-field, they may decide to use different sub-$\sigma$-fields and report differing predictions. 
...
\end{minipage}
\end{figure}
The relevant relations can be summarized by a multivariate Gaussian distribution:
\begin{align}
...
\delta$, $|B_{i} \cap B_{j}| = \lambda \delta$, where $\delta$ is the amount of information used by each forecaster and
...
forecasters used information sources sampled from a common distribution. 
...
the forecasters partition the total information with zero information overlap, i.e., $\delta =
...
information used by the forecasters are important yet separate determinants of  extremizing. This observation can guide practitioners towards proper extremization because  many  application specific aspects can be linked to these two determinants. For instance, extremization can be
...
Under the Gaussian model, exchangeability suggests the compound
