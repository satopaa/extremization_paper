...
information structure to many known results in combinatorics and geometry. This paper discusses one such connection in Proposition \ref{CorrelationPolytope} and another in Section \ref{oracular}. Second, there is
...
\end{cases}
\end{align*}
for $i,j,k \in \{1,2,3\}$ such that $i \neq j$, $i \neq k$, and $j \neq k$. After this, the inequality representation of $\COR(N)$ becomes increasingly complex. For instance, $\COR(5)$ is known to have 56 facets while $\COR(9)$ has at least 12,246,651,158,320 facets. In fact, because the unconstrained quadratic
...
|B'|$. Similarly to a single forecaster's probability estimate in (\ref{indFore}), the oracular aggregator becomes
 \begin{align*}
p' = \P(X_S > 0 |  \mathcal{F}') 
   &= \Phi\left( \frac{X_{B'}}{\sqrt{1-\delta'}} \right)
\end{align*}

Given that $p'$ is generally not observed, $\delta'$ cannot be estimated directly but must be computed based on the forecasters' information $\Sigma_{22}$ instead. The value of $\delta'$, however, is not uniquely determined by $\Sigma_{22}$ unless $N = 2$. 
%The a key quantity in analyzing other probability aggregators is the oracle's amount of information $\delta'$. Generally this value is determined by $\Sigma_{22}$ only when $N = 2$.
Fortunately, it can be bounded in terms of $\Sigma_{22}$ for general $N$ by  referring to the historical Boole Problem (\citealt{boole1854investigation}): 
...
%\delta'_{min} := 
\frac{N \sum_{i=1}^N \delta_i - 2 \sum_{1 \leq i < j \leq N} \rho_{ij}}{\lfloor \frac{N+1}{2} \rfloor \lceil \frac{N+1}{2} \rceil }  \leq \delta' \leq \frac{N \sum_{i=1}^N \delta_i - 2 \sum_{1 \leq i < j \leq N} \rho_{ij}}{N} 
%:= \delta'_{max}
\end{align*}
and also provide an excellent overview of other existing upper and lower bounds. 
%where 
%\begin{align*}
...
Such results make $p'$ a tangible benchmark that can be used to analyze other aggregators under different information structures $\Sigma_{22}$. Approximating or bounding $\delta'$, however, is not always necessary. This is illustrated in the following subsections that do not approximate $\delta'$ but instead use known relations among the parameters in (\ref{oracleN}) to develop intuition about probability extremizing. 

\subsection{General Information Structure}
 A
probability $p$ is said to be \textit{extremized} by another probability $q$ if and only
if $q$ is closer to $0$ when $p \leq 0.5$ and closer to $1$ when $p
\geq 0.5$. Equivalently, $q$ extremizes $p$ if and only if its probit score $\Phi^{-1}(q)$ is on the same side but further away from zero than $\Phi^{-1}(p)$. The amount of (multiplicative) extremization can then be quantified with the {\em probit extremization ratio} defined as 
...
%symmetric}. 
As a result, the multivariate Gaussian distribution
(\ref{oracleN}) reduces to
...
The overlap parameter $\lambda$ must be constrained in order to ensure a coherent information structure.
First, the upper bound is $\lambda \leq 1$ because under any combination of $\delta$ and $N$ the
forecasters may know the exact same information. Second, 
information overlap is unavoidable when $\delta > 1/N$, and minimum
...
\delta \in [0,1] &&  \lambda &\in \left[  
   ...
   \label{rhoDomain}
\end{align}
where the open upper bound on $\lambda$ ensures a non-singular $\Sigma_{22}$. Under these simplifications, the extremization ratio (\ref{alpha})
becomes
\begin{align*}
...
\caption{$\P(\alpha \geq 1 | \Sigma_{22})$}
\label{probOracle}
        \end{subfigure}

        \caption{ The amount of extremizing follows a Cauchy$(x_0, \gamma)$, where $x_0$ is a location parameter and $\gamma$ is a scale parameter. This figure considers  $N = 2$ experts and shows $\log(x_0)$ and $\P(\alpha \geq 1 | \Sigma_{22})$ under all plausible combinations of $\delta$ and $\lambda$. No approximation is needed as $\delta'$ is uniquely determined by $\Sigma_{22}$ when $N = 2$.}
        \label{LevelplotsOracle}
\end{figure}

Recall that $\delta'$ increases in $N$ and $\delta$ but decreases in $\lambda$. More specifically, as $\delta \to 1$, $\delta' \to 1$ but at a faster rate as long as $\lambda < 1$ and $N$ are fixed. Therefore $x_0$ generally increases in $N$ and $\delta$ but decreases in $\lambda$; or alternatively, the amount of extremizing
can be expected to increase in $\delta'$.  Furthermore, as the Cauchy distribution is symmetric around $x_0$, the probability $\P(\alpha \geq 1 | \Sigma_{22})$ behaves similarly to $x_0$ and hence also increases in $\delta'$. 
%The rate of increase, however, is not the same as for $x_0$ because it also depends on $\gamma$. 
Figure \ref{LevelplotsOracle} illustrates these relations by plotting both $\log(x_0)$ and $\P(\alpha \geq 1 | \Sigma_{22})$ for $N = 2$ forecasters under all plausible combinations of $\delta$ and $\lambda$. High values have been censored to keep the scale manageable. Note that the results are completely general for the two-forecaster case, apart from the assumption $\delta_1 = \delta_2$. Relaxing this assumption, however, does not change the qualitative nature of the results.
...
reconsider Figure \ref{LevelplotsOracle} and observe that fixing $\delta'$ to
some constant defines a curve $\lambda = 2 - \delta'/\delta$ on the two plots. For
...
moving down along these curves increases information diversity and
$x_0$, both information diversity and the total amount of
information in the group are important 
 determinants of extremizing. This observation can guide practitioners towards proper extremization by linking  application specific aspects of the prediction problem to the two aforementioned determinants. For instance, extremization can be
...
practice, and also provides some empirical evidence in favor of the
