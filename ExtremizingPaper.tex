...
The interpreted signal framework, as proposed, is relatively abstract; Our partial information model formalizes the intuition behind it, and allows quantitative predictions. In the absence of a quantitative interpreted signal model, people have typically explained subjective forecasts with standard statistical models. These have been called \textit{measurement error framework}; we describe it in the next subsection.
...
\item {\bf Pool of Information.} First, the role of the unit interval is for
...
could have been used. The unit interval, however, is a natural starting point that allows us to treat $\{ |B_j| : j = 1, \dots, N\}$ as probabilities for some $N$ events, $\{ |B_i \cap B_j| : i \neq j\}$ as their pairwise joint probabilities, $\{ |B_i \cap B_j \cap B_k| : i \neq j \neq k\}$ as their three-way joint probabilities, and so on.  This is particularly useful in analysis as it links the information structure to many known results. One such connection is established in Proposition \ref{CorrelationPolytope}. Second, there is no sense of time or ranking of the
...
covariance matrix under several restrictions.  In particular, the information in matrix $\Sigma_{22}$ must be descriable by a diagram such as the one
given in Figure \ref{diagramN}. If this is the case, the matrix $\Sigma_{22}$ is called \textit{coherent}.  Coherence clearly requires each element of
$\Sigma_{22}$ to be in the unit interval with no off-diagonal
element being larger than the corresponding diagonal element in the
same row or column.  These conditions, however, are not sufficient.  The following proposition provides the remaining details by describing the space of coherent information structures. The proof treats $\{\delta_j : j = 1, \dots, N\}$ as probabilities for some $N$ events (recall Remark (\ref{item:pool})) and therefore regards Figure \ref{diagramN} as a type of Venn diagram. The details of this and other proofs are deferred to the Appendix.

\begin{proposition}
\label{CorrelationPolytope}
The overlap structure $\Sigma_{22}$ is coherent if and only if $$\Sigma_{22} \in  \text{COR}(N) := \text{conv}\left\{ \boldsymbol{x}\boldsymbol{x}' : \boldsymbol{x} \in \{0,1\}^N\right\},$$ where $\text{COR}(N)$ is known as the correlation polytope or the boolean quadric polytope. It is described by $2^N$ vertices in dimension $\text{dim}(\text{COR}(N)) = \binom{N+1}{2}$.  
\end{proposition}
The correlation polytope is a known to have a very complex $\mathcal{H}$-representation, i.e. a description in terms of a finite number of half-spaces. In fact, because the unconstrained quadratic 0-1 problem is NP-hard, it is probably hopeless to find a complete list of linear inequalities that describe the correlation polytope (\citealt{deza1997geometry}). Approximating the correlation polytope and estimating the general information structure, however, are not in the scope of this paper and  are therefore left for future work. 
...
\end{align}
From now on, unless otherwise stated, we refer to (\ref{alpha}) simply with
...
The amount of information known by the oracle $\delta'$ is uniquely determined by $\Sigma_{22}$ only when the group involves only two experts. More specifically, in this case the inclusion-exclusion principle gives that $\delta' = \delta_1 + \delta_2 - \rho_{ij}$. Fortunately, Proposition \ref{CorrelationPolytope} connects the information structure $\Sigma_{22}$ with the historical Boole Problem (\citealt{boole1854investigation}) and therefore provides many well-known bounds for $\delta'$ when $N \geq 2$. For instance, one choice is
\begin{align*}
\delta'_{min} := \frac{N \sum_{i=1}^N \delta_i - 2 \sum_{1 \leq i \leq j \leq N} \rho_{ij}}{\lfloor \frac{N+1}{2} \rfloor \lceil \frac{N+1}{2} \rceil }  \leq \delta' \leq \frac{N \sum_{i=1}^N \delta_i - 2 \sum_{1 \leq i \leq j \leq N} \rho_{ij}}{N} := \delta'_{max}
\end{align*}
%where
%\begin{align*}
...
%\end{align*}
See \citealt{deza1997geometry} for the discussion of these and other bounds. Notice that both bounds are equal to $\delta_1 + \delta_2 - \rho_{ij}$ when $N=2$. The lower bound is particularly useful because it allows us to lower bound the amount of extremization under any number of forecasters. 

First, however, the number of parameters in $\Sigma_{22}$ must be reduced. A natural approach is to assume that the
...
Plugging this structure in the multivariate Gaussian distribution (\ref{oracleN}) gives
...
Under these simplifications, the extremization ratio (\ref{alpha}) becomes
\begin{align*}
...
\end{align*}
The location parameter $x_0$ can be lower bounded by replacing $\delta'$ with $\delta'_{min}$. Making this substitution and recalling Proposition \ref{positiveProbThm} gives us 
\begin{align}
...
\end{figure}
 Notice that extremizing is required more
often when $\delta$ and $N$ are high but $\lambda$ is low.  Given that
$\delta'$ increases in $\delta$ and $N$ but decreases in $\lambda$, the amount
...
this, consider Figure \ref{xOracle2} and observe that fixing $\delta'$ to some constant defines a curve
$\lambda = 2 - \delta'/\delta$. For instance, letting $\delta' = 1$
gives the boundary curve on the right side of the plot.  This curve
...
%\begin{observation}
This guides practitioners towards proper extremization because many aspects of the prediction context can be viewed closely related to these two factors. For instance,  extremization can be expected to increase in factors such as
...
Under the Gaussian partial information model, this implies a compound symmetric information structure (see Section \ref{compound}). The general form of the revealed
aggregator~(\ref{GeneralAggregator}) then simplifies to
\begin{align}
p''
...
\citealt{mellers2014psychological, ungar2012good}) is a research study that since 2011 has recruited thousands of
forecasters from professional societies, research centers, and alumni
associations.  These forecasters are given questions about future
...
are assessed for accuracy using Brier scores (a squared error loss function).  % This incentivizes
...
scores for the top 20 forecasters.  Every year the top 1\% percent of
the forecasters are selected to the elite group of
``super-forecasters''. The super-forecasters work in groups to make highly accurate predictions on the same events as the rest of the forecasters.
...
likely, but not guaranteed, to be relatively good. This group consisted of 44 super-forecasters making predictions about
...
\end{align*}
to the space of all coherent information structures $\text{COR}(N)$. In particular, the linear function $L: \boldsymbol{z} \in \Delta_N \to \Sigma_{22} \in \text{COR}(N)$ is defined such that $\rho_{ij} = \sum_{\{i,j\} \subseteq v} z_v$ and $\delta_i =  \sum_{i \in v} z_v$. Given that $\Delta_N$ is a convex polytope, transforming it via the linear function yields a convex polytope spanned by the $2^N$ points $\{ L(\boldsymbol{e}_v) : v \subseteq \{1, \dots, N\}  \} = \{\boldsymbol{x}\boldsymbol{x}' : \boldsymbol{x}  \in \{0,1\}^N\}$. That is, $L(\Delta_N) = \text{COR}(N)$, which is known as the boolean quadratic polytope, a.k.a. correlation polytope. Each $\Sigma_{22} \in \text{COR}(N)$ has $\frac{N(N+1)}{2} = \binom{n+1}{2}$ unique values and therefore exists in $\binom{n+1}{2}$ dimensions. \qed
