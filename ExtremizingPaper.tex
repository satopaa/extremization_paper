...
\begin{minipage}[t]{.5\textwidth}
	\hspace{2em}\includegraphics{LegendMarginal} % requires the graphicx package
...
\end{align}
Let $\alpha$ represent the amount of extremization that is performed for the average probit forecast. If $\bar{P} = \left( \sum_{j=1}^N P_{I_j} \right)/N$, then
\begin{align}
...
\end{align}
which follows from p. 32 in \cite{rao2009linear}. As the compound symmetric information structure depends only on two unknown parameters, the values of $\delta$ and $\rho$ can be estimated in practice via the maximum likelihood method. This requires a closed form expression for the density of $\boldsymbol{p}$. First, note that $\boldsymbol{P} \sim \mathcal{N}_N(\boldsymbol{0}, \Sigma_{22} (1-\delta)^{-1})$ and that the Jacobian for the map $\boldsymbol{P} \to \Phi\left(\boldsymbol{P}\right)$ is
\begin{eqnarray*}
J(\boldsymbol{X}) &=& (2\pi)^{-N/2} \exp \left( - \frac{\boldsymbol{P}' \boldsymbol{P}}{2}   \right) 
\end{eqnarray*}
If $h(\boldsymbol{P})$ denotes the multivariate normal density of $\boldsymbol{P}$, then 
...
\end{eqnarray*}
where $\Sigma_{22}^{-1}$ and $|\Sigma_{22}|$ are given by (\ref{inverse}) and (\ref{determinant}), respectively. The maximum likelihood estimates of $\delta$ and $\rho$ are then obtained from
\begin{align}
...
Equating this with $\bar{P}$ results in the following extremization factor.
\begin{align}
...
\begin{align*}
\left(\begin{matrix} \boldsymbol{X}_S \\ \boldsymbol{X}^{(1)}\\ \vdots \\ \boldsymbol{X}^{(N)} \end{matrix}\right) &\sim \mathcal{N}\left( 
 \boldsymbol{0}, \left(\begin{matrix} 
...
\Lambda_{21} & \Lambda_{22}\\
 ...
 \left(\begin{array}{c c c | c c c c c}
  \multicolumn{3}{c|}{\multirow{3}{*}{$I_{K-1}$}} & \Sigma_{12}  & \boldsymbol{0} & & \multirow{2}{*}{$\boldsymbol{0}$}  \\ 
 &  & & \multirow{2}{*}{$\boldsymbol{0}$}&  \Sigma_{12} & \dots & \\ 
  &  &  &  & \boldsymbol{0}  &&\Sigma_{12}  \\ \hline
\Sigma_{21} & \multicolumn{2}{c|}{\boldsymbol{0}} & \Sigma_{22} & & \multicolumn{2}{c}{\multirow{2}{*}{$\boldsymbol{0}$}}   \\ 
\boldsymbol{0}_{} & \Sigma_{21} & \boldsymbol{0} &  & \Sigma_{22} &&  \\ 
\multicolumn{3}{c|}{\vdots} &\multicolumn{2}{c}{\multirow{2}{*}{$\boldsymbol{0}$}}  & \ddots &  \\ 
\multicolumn{2}{c}{\boldsymbol{0}}  & \Sigma_{21} &  &  &&\Sigma_{22} \\ 
 \end{array}\right)\right),
\end{align*}
...
\begin{align*}
\boldsymbol{\bar{\mu}} &=  \Sigma_{12} \Sigma_{22}^{-1} \boldsymbol{X} &&  \bar{\Lambda}= (1 - \Sigma_{12} \Sigma_{22}^{-1} \Sigma_{21}) I_{K-1}
\end{align*}
...
The $k$th cone is then the convex cone defined by all vertex points except the $k$th point. More specifically, the $k$th cone is given by
\begin{align*}
...
\end{align*}
The aggregate probability for the $k$th outcome is computed by integrating over the $k$th cone with respect to the conditional distribution of $\boldsymbol{X}_S$. This integration is much easier after transforming the cone is into the convex cone defined by the first $K-1$ standard basis functions. Denote this standard cone with $C_e$ and let 
\begin{align*}
R &= \left(\begin{matrix} 
\boldsymbol{b}_1\\
\vdots\\
\boldsymbol{b}_{k-1}\\
 ...
 \end{matrix}\right) ^{-1},
\end{align*}
If $\boldsymbol{\tilde{X}} = R\boldsymbol{X}_S$, then $R\boldsymbol{X}_S | \boldsymbol{X}  \sim \mathcal{N}(R\boldsymbol{\bar{\mu}}, R' \bar{\Lambda}R)$

\begin{align*}
\P(\boldsymbol{X}_S \in C_k | \boldsymbol{X}) &= \P(R \boldsymbol{X}_S \in C_e | \boldsymbol{X})\\
&= \Phi_{K-1} \left(\left(1 - \Sigma_{12} \Sigma_{22}^{-1} \Sigma_{21}) I_{K-1}\right)^{-1/2}R\Sigma_{12} \Sigma_{22}^{-1} \boldsymbol{X} \right)
\end{align*}
