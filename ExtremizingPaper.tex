...
Our first contribution is to develop a concrete model for probability forecasts based on the \textit{partial information framework} (see the accompanying paper). Under this framework the experts turn personal information into probability forecasts. Therefore any forecast heterogeneity stems from differences in the experts' private information sets. This preserves a close connection with the real world while permitting  models that can be estimated in practice. For instance, our model for probability forecasts represents  the experts' information with a covariance matrix that can be constrained in different ways to suit the available resources. 

Our second contribution is to remedy the aforementioned drawbacks of the current aggregators. First,  an entire class of aggregators is derived based on the partial information model. The members of this class are indexed by the information among the experts. This establishes a link between the amount of extremization performed for naive aggregates, such as average probit- and logit-forecasts, and different information structures among the experts. The result is a more principled understanding of extremization that can be interpreted in terms of information diversity. Second, our close analysis of extermination reveals two main sources of \textit{reverse-extremization} (shifting the average probability away from its closest extreme). This leads an information structure that is immune to reverse-extremization. The corresponding aggregator always extremizes and depends only on two intuitive parameters that can be learned from the experts' forecasts without a separate training set. 
