...
Event forecasting is the science of giving probability estimates for future events.  Even though the classical examples can be found in meteorology
\citep{sanders1963subjective}, where the interest is typically in predicting different weather
phenomena such as rain in the next few days, the practice has 
...
this year \citep{tetlock2005expert}. Accurate probability forecasts are particularly important to the decision-maker who must decide, for instance, what to pack
...
is based on local news in Egypt. Instead of trying to guess which one of the forecasts is more accurate, the decision-maker should combine the forecasts because, based on much past research, an aggregate forecast is typically more accurate than the individual forecasts \citep{clemen1989combining,
...
rule can largely affect the predictive quality of the final
aggregate.  
%Analyzing and developing improved rules is the main motivation of 
This motivates the problem of  \textit{forecast aggregation} that aims to combine multiple forecasts into a single
...
\label{BiasNoise}
For the sake of theoretical development, it is convenient to represent the target event with an
...
estimator of this indicator.  Therefore, as is the case with
...
\label{PIFintro}
This section introduces the partial information framework at the most general level. As is the case with any probability model for forecast aggregation, the construction must begin with
...
the expectation of any proper loss function (\citealt{Ranjan08}). Furthermore, just like any conditional expectation of $\one_A$ given a sub-$\sigma$-field, the revealed aggregator is calibrated. Similarly, $p'$ and $p_i$ are also calibrated. Unlike the oracular aggregator, however, $p''$ can be applied in practice and is therefore the relevant estimator in each specific partial information model. 
...
To the best of our knowledge, no previous work has provided a general framework that links the interpreted forecasts to their target quantity. Therefore the interpreted signal framework, as proposed, has remained relatively abstract. 
%The interpreted signal framework, when formalized, more or less implies the partial information framework.
The partial information framework, however, formalizes the intuition behind it, allows quantitative predictions, and provides a flexible construction that can be adapted to a broad range of forecasting setups. 
...
$\theta$  for the future event to be predicted. This probability, which can be interpreted as the  forecast made by an ideal forecaster, is then measured by the individual forecasters with mean-zero idiosyncratic error.  Therefore each forecast is an independent draw from a common
...
expected from an aggregator based on a model that not does link the observations and the correct quantity
of interest.
...
too close to $0.5$. This result applies to the averaging aggregators that are by nature different convex combinations of the
forecasts.
...
aggregator extremizes $\bar{p}$ and has some attractive theoretical
...
directions.  One involves estimation of parameters.  A partial information model together with good procedures for estimating
