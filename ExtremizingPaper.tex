...
Therefore,
forecast heterogeneity does not only stem from differences in information available to the forecasters, but also from how they decide to use it.  Note, however, that if a forecaster chooses to discard
some of the available information, then, for the purposes of aggregation,
that information may as well not exist and, consequently, $\F_i$ can be interpreted as the full information known to Forecaster $i$.
...
generated by the union of the information sets $\{\F_i : i = 1, \dots, N\}$. Therefore any
information that is not in $\F'$ represents randomness inherent in the
outcome. Given that aggregation cannot be improved beyond using all of the forecasters' information, the oracular aggregator represents a theoretical optimum and is therefore a
reasonable upper bound on estimation efficiency. 
...
This is jointly characterized in the following multivariate Gaussian distribution:
\begin{align}
