...
\textcolor{red}{include the covariance motivation here.}
...
\item The modal amount of extremizing is increasing in the level of information diversity and the total amount of information among experts.

\item The amount of extremizing establishes a spectrum from full information overlap/no extremization to no information overlap/full extremization. The two extremes require different aggregation techniques. In particular, average aggregates are likely to work well under high information overlap and more extreme aggregates such as voting are likely to dominate under low information overlap.
\end{itemize}
...
\end{figure}
 ...
 Even though the pool of information is indexed by the unit interval, it is important to emphasize that there is no sense of time or ranking of information. Instead the pool is a collection of information, where each piece of information has \textit{a priori} an equal chance to contribute to the final outcome of the event. It is not necessary to assume anything about the source or form of the information. However, it may be helpful to keep in mind the website example described in the introduction. Under this example, the unit interval $S$ represents the set of all relevant websites, the subinterval $I_j$ denotes the set of websites visited by expert $E_j$, and the information overlap $I_1 \cap I_2$ is the set of websites visited by both experts. The incremental information on a single website has an additive effect which is either negative (makes $A$ less likely) or positive (makes $A$ more likely) such that the sum of contributions across all websites yields the final $X_S$. 
...
This section assumes that the experts' information sets have the same size and that the amount of overlap between any two information sets is constant, i.e.  $|I_{1}| =  \dots = |I_{N}|$ and $|I_{i} \cap I_{j}| = |I_{h} \cap I_{k}|$ for all $i \neq j$ and $h \neq k$. Therefore each expert knows and shares the same amount of information with every other expert. The corresponding information structure $\Sigma_{22}$ is compound symmetric. This structure arises if the experts' choose to include a particular information source with a common probability. For instance, the expert may choose to visit a website proportional to the total number of past visits to that page. If all experts choose their information sources in this manner, the information structure is, in expectation, compound symmetric. That is,
...
This all suggests a spectrum of aggregators indexed by the informative overlap. Under full information overlap, averaging the probit forecasts yields an optimal aggregate. As the overlap decreases towards the point of no overlap, however, the optimal aggregator changes from averaging to summing of the probit forecasts. Therefore forecasts from a group of experts working together can be averaged while forecasts from a diverse group of experts working independently of each other should be aggregated via more extreme techniques such as voting. 
