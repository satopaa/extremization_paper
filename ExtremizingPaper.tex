...
This paper begins in Section 2 by introducing the partial information model for probability forecasts. Section 3 develops the class of aggregators, describes the amount of extremization for each member of the class, and illustrates two typical sources of reverse-extremization (shifting the average probability away from its closest extreme). This leads an information structure that is immune to both sources of reverse-extremization. The corresponding aggregator always extremizes and depends only on two intuitive parameters that can be learned from the experts' forecasts without a separate training set. The amount of extremization performed by this aggregator has a simple form that is studied graphically at the end of Section 3. The paper concludes in Section 4 with a discussion of the main results, model limitations, and future directions. 
...
\end{align}
Given that $X_S$ has mean zero, $\P(X_S > 0) = \P(A) = 0.5$. This can be viewed as the decision maker's prior probability of $A$ and is easily adjusted by changing the event $A$. That is, if the decision maker's prior belief is denoted with $\tilde{p}$, then the event $A$ should be defined as $A = \{ X_S > \Phi^{-1}(1-\tilde{p}) \}$, where $\Phi$ represents the standard Gaussian CDF. As this paper is not concerned with any particular problem, the prior belief is taken non-informative, i.e. $\tilde{p} = 0.5$.  
...
To link this model with the probability forecasts, denote the probit-forecast of the $j$th expert with $P_{I_j} = X_{I_j}/\sqrt{1-\delta_j}$.  The probability forecast given by the $j$th expert is then
\begin{align}
...
\end{align}
This aggregator can be described in two steps: The first step is voting where votes are weighted according to the importance of the corresponding experts' private information. This step is performed by the summation in the numerator. If this sum falls below $0.0$ (or above $0.0$), the consensus believes that the event will not happen (or will happen). The second step extremizes this belief according to the total amount of information in the group. For instance, if the experts know all the information, i.e. $\sum_{j=1}^N \delta_j = 1$, their vote deterministically indicates whether the event $A$ happens or not. This step is performed by the denominator. Overall, the procedure resembles voting. Therefore voting can be expected to work well in practice when the voters form a very knowledgable and diverse group of experts.  The next example shows that this aggregator does not always extremize by discussing a second source of reverse-extremization.
...
In practice the values of $\delta$ and $\rho$ can be estimated via the maximum likelihood method. This requires a closed form expression of the density of $\boldsymbol{p} = (p_1, p_2, \dots, p_N)$. To derive this, notice that $\boldsymbol{P} \sim \mathcal{N}_N\left(\boldsymbol{0}, \Sigma_{22}^{(cs)} (1-\delta)^{-1}\right)$ and that the Jacobian for the map $\boldsymbol{P} \to \Phi\left(\boldsymbol{P}\right)$ is
\begin{eqnarray*}
...
% 1. In the real world there does not seem to be the two sources of reverse-extremization.
% 2. Voting works well when (move it here)
% 3. Illustrated that parameters can be estimated
