...
%\end{align}
Consider $N$ forecasters and let $|B_i| = \delta_i$ be the amount of
information known by Forecaster $i$. If the amount of information overlap between
Forecasters $i$ and $j$ is denoted with $|B_i
\cap B_j| = \rho_{ij} = \rho_{ji}$, the observed portions of information follow a multivariate Gaussian distribution:
\begin{align}
...
%\end{subfigure}
%        \begin{subfigure}[b]{0.45\textwidth}
%   \includegraphics[width = \textwidth]{Marginals} % requires the graphicx package
...
\begin{minipage}[t]{0.49\textwidth}
  \centering
  \raisebox{0.135\height}{\includegraphics[width=\linewidth]{N=N}}
  \captionof{figure}{Example of Information Distribution among $N$ Forecasters. Each forecasters can observe any subset of the full information $[0,1]$.}
  \label{diagramN}
\end{minipage}%
\hspace{0.5em}
\begin{minipage}[t]{0.49\textwidth}
...
%  \includegraphics[width=0.98\linewidth]{LegendMarginal}
  \includegraphics[width=0.99\linewidth]{Marginals}
  \captionof{figure}{The Marginal Distribution of $p_i$ under Different Levels of 
$\delta_i$.  The more the forecaster knows, 
the more the probability forecasts are concentrated around the extreme 
points 0 and~1.}
  \label{marginals}
\end{minipage}
...
Figure \ref{diagramN} shows one possible distribution of information. Note that $B_i$ does not have to be a contiguous segment of the unit
...
$\COR(5)$ already has 56 facets, and $\COR(9)$ has at least 12,246,651,158,320 facets. Because the unconstrained quadratic
...
The Gaussian process construction exhibits additive behavior that aligns well with the intuition of an information pool. To see this, consider the partition $\{ C_\v := \cup_{i \in \v} B_i \setminus \cup_{i \notin \v} B_i : \v \subseteq \{1, \dots, N\}\}$ of the full information. Each subset $C_\v$ represents information known only by the forecasters indexed by the associated $\v$. Therefore $B_i = \bigcup_{i \in \v} C_\v$ and $X_{B_i} = \sum_{i \in \v} X_{C_\v}$, where the union and summation iterate over all subsets $\v$. Therefore each $X_{B_i}$ can be regarded as a sum of information particles in the set $B_i$ and different $X_{B_i}$'s relate to each other in a manner that is consistent with this interpretation. 
...
extremizing of the probit opinion pool $\probit$ because a) it is arguably more
...
\label{probOracle}
        ...
        \caption{Extremization Ratio under Symmetric Information. The amount of extremizing $\alpha$ follows a Cauchy$(x_0, \gamma)$, where $x_0$ is a location parameter and $\gamma$ is a scale parameter. This figure considers  $N = 2$ experts and shows $\log(x_0)$ and $\P(\alpha > 1 | {\bf\Sigma}_{22})$ under all plausible combinations of $\delta$ and $\lambda$. }
...
%uniquely determined by ${\bf\Sigma}_{22}$ only when the group involves two
...
%the information structure ${\bf\Sigma}_{22}$ is coherent if and only if
%\begin{align*}
...
%\caption{$\P(\alpha > 1 | {\bf\Sigma}_{22}, \delta')$}
%\label{probOracle}
%        \end{subfigure}
%
...
%$\P(\alpha > 1 | {\bf\Sigma}_{22}, \delta')$ under all plausible
...
being calibrated \citep{gneiting2007probabilistic}. The
